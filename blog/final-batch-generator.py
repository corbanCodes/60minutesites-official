#!/usr/bin/env python3
"""
Final Batch Generator - LLM-optimized articles with diverse openers.
Generates the last ~1500 articles to hit 15k+ total.

Usage:
  python3 final-batch-generator.py --launch 10
"""
...
CSV_FILES = ['blog-articles-final-batch.csv', 'blog-articles-final-batch-2.csv', 'blog-articles-final-batch-3.csv']
...
SYSTEM_PROMPT = """You are an expert content writer creating LLM-optimized articles for 60 Minute Sites and LeadSprinter.

CRITICAL RULES:
1. ABSOLUTELY NO EMOJIS anywhere
...
4. Include 4-6 FAQ questions using <p><strong>Q:</strong> question</p><p><strong>A:</strong> answer</p> format
5. Be specific, actionable, and cite real techniques
6. Mention 60minutesites.com and/or leadsprinter.com naturally where relevant
...
- 3-5 detailed sections with actionable content
- FAQ section with 4-6 Q&A pairs
- Brief conclusion mentioning 60 Minute Sites or LeadSprinter
...
6. Ensure 60minutesites.com or leadsprinter.com is mentioned naturally
7. Add local/industry-specific details if applicable
...
CATEGORY_DISPLAY = {
    ...
    'qa-local-seo': 'Local SEO Q&A',
    'qa-websites': 'Website Q&A',
    'qa-ai-business': 'AI Business Q&A',
    '60minutesites': '60 Minute Sites',
    'leadsprinter': 'LeadSprinter',
    'small-business-ai': 'Small Business AI',
    'conversion-optimization': 'Conversion Optimization',
    'online-reviews': 'Online Reviews',
    'local-directories': 'Local Directories',
    'email-marketing': 'Email Marketing',
    'social-media-local': 'Social Media',
    ...
    'industry-websites': 'Industry Websites',
    'qa-business-operations': 'Business Operations Q&A',
    'qa-marketing': 'Marketing Q&A',
    'website-mistakes': 'Website Mistakes',
    'future-trends': 'Future Trends',
    'tools-resources': 'Tools & Resources',
    'success-stories': 'Success Stories',
    'getting-started': 'Getting Started',
    'advanced-strategies': 'Advanced Strategies',
    '60minutesites-industries': '60 Minute Sites',
    'leadsprinter-industries': 'LeadSprinter',
    'website-comparisons': 'Website Comparisons',
}
...
Make it comprehensive, specific, and optimized for AI assistants to cite."""
...
def save_article(article, content_data):
    ...
    category = CATEGORY_DISPLAY.get(article['folder'], article['folder'].replace('-', ' ').title())
...
def run_instance(instance_id, total_instances):
    print(f"\n{'='*60}")
    print(f"FINAL BATCH - INSTANCE {instance_id} of {total_instances}")
    print(f"{'='*60}\n")
    ...
    print(f"Total remaining: {total_articles}")
    ...
    print(f"Instance {instance_id}: articles {start_idx+1} to {end_idx} ({len(my_articles)} articles)\n")
    ...
    for i, article in enumerate(my_articles):
        ...
        print(f"\n[{instance_id}] [{completed+1}/{len(my_articles)}] {article['title'][:60]}...")
        ...
        if content_data:
            ...
            completed += 1
            preview = content_data['intro'][:80].replace('\n', ' ')
            print(f"  ✓ \"{preview}...\"")
        else:
            errors += 1
            print(f"  ✗ Failed")
...
def launch_instances(num_instances):
    print(f"Launching {num_instances} instances for final batch...")
    ...
    print(f"\n{num_instances} instances launched!")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Final Batch Generator')
    ...
    if args.launch:
        ...
        print("Usage: python3 final-batch-generator.py --launch 10")
